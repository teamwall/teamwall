...
(facts "About routes"
  ...
  (fact "team-members returns a 200 and a JSON object
        if correct token is provided"
    (against-background
     ...pattern... =contains=> ""
  ...
  (fact "last-photo returns a 400
        if the email provided does not match the user email domain"
    (against-background
     ...token... =contains=> "correct token"
     (#'teamwall.handler/get-user-for-token ...token...) => {:email "wrong"}
     (#'teamwall.handler/valid-token? anything
                                      ...token...) => true)
    (let [response (app-routes (mock :get
                                     "/john@doe.com/last-photo"
